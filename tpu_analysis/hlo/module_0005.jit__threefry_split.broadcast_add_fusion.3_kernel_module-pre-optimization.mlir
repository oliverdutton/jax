module @broadcast_add_fusion.3_kernel_module attributes {dlti.dl_spec = #dlti.dl_spec<index = 64 : i32>, xla.cpu_memory_region_name = "xla_cpu_emitter__loop_fusion_kernel_emitter__hlo_opcode__fusion"} {
  func.func @broadcast_add_fusion.3(%arg0: tensor<2xi32> {llvm.align = 64 : index, llvm.dereferenceable = 8 : index, xla.invariant, xla.slice_index = 0 : index}, %arg1: tensor<3xi32> {llvm.align = 64 : index, llvm.dereferenceable = 12 : index, xla.slice_index = 1 : index}) -> tensor<3xi32> attributes {xla.backend_kind = #xla.backend_kind<cpu>, xla.entry} {
    ...
    %3 = scf.forall (%arg2, %arg3, %arg4) in (1, 1, 1) shared_outs(%arg5 = %arg1) -> (tensor<3xi32>) {
      %xla_loop = xla.loop (%arg2, %arg3, %arg4, %0, %1, %2)[%i] -> (%ra) in #xla.indexing_map<"(th_x, th_y, th_z, bl_x, bl_y, bl_z)[s0] -> (s0), domain: th_x in [0, 0], th_y in [0, 0], th_z in [0, 0], bl_x in [0, 0], bl_y in [0, 0], bl_z in [0, 0], s0 in [0, 2]"> iter_args(%iter = %arg5) -> (tensor<3xi32>) {
        %pure_call = xla.pure_call @fused_computation_6_add_92(%arg0, %ra) : (tensor<2xi32>, index) -> i32
        %inserted = tensor.insert %pure_call into %iter[%ra] : tensor<3xi32>
        xla.yield %inserted : tensor<3xi32>
      }
      scf.forall.in_parallel {
        tensor.parallel_insert_slice %xla_loop into %arg5[0] [3] [1] : tensor<3xi32> into tensor<3xi32>
      }
    }
    return %3 : tensor<3xi32>
  }
  func.func private @fused_computation_6_add_92(%arg0: tensor<2xi32>, %arg1: index {xla.range = [0 : index, 2 : index]}) -> i32 attributes {llvm.linkage = #llvm.linkage<internal>} {
